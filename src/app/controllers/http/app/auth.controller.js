...
module.exports = {
    ValidateToken: async (req, res) => {
        const validator = require("@Middwares/app/params/auth/validate_token")(req.query);
    ...
    LoginAccount: async (req, res) => {
        const validator = require("@Middwares/app/params/auth/validate_login_account")(req.body);
        ...
        try {
            ...
            const user = await AppUserModel.findByUsername(username);
    ...
    RegisterAccount: async (req, res) => {
        const validator = require("@Middwares/app/params/auth/validate_register_account")(req.body);
        if (!validator.stopOnFirstFailure().validate()) return res.status(200).json(RESP_DATA.ERROR(
            CODE_ENUM.ERROR_FORM.INVALID_FIELD,
            TEXT_ENUM.ERROR_FORM.INVALID_FIELD,
            validator.errors().all()
        ...
        try {
            ...
            if (await AppUserModel.findByUsername(user.username)) return res.status(200).json(RESP_DATA.ERROR(
                CODE_ENUM.ERROR_AUTH_MESSAGE.UsernameExists,
                TEXT_ENUM.ERROR_AUTH_MESSAGE.UsernameExists
            ));
            // check exist account same phone
            if (await AppUserModel.findByPhoneNumber(user.phone)) return res.status(200).json(RESP_DATA.ERROR(
                CODE_ENUM.ERROR_AUTH_MESSAGE.PhoneExists,
                TEXT_ENUM.ERROR_AUTH_MESSAGE.PhoneExists
            ));
            // check exist account same email
            if (await AppUserModel.findByEmail(user.email)) return res.status(200).json(RESP_DATA.ERROR(
            ...
            user.verify = AppUserModel.VERIFY_ENUM.FALSE;
            user.status = AppUserModel.STATUS_ENUM.ACTIVE;
            const userSaved = await AppUserModel.create(user);
